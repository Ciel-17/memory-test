...
// ========== 总结策略演示(基于 token 数量) ==========
async function summerizationMemoryDemo(){
    const history = new InMemoryChatMessageHistory()
    const maxTokens = 200 // 超过200个token进行总结
    ...
    const totalTokens = countTokens(messages, enc);
...
summerizationMemoryDemo().catch(console.error);